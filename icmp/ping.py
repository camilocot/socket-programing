...
# https://github.com/godaddy/netmet/blob/master/netmet/utils/ping.py
...
class Ping(object):
    ...
    def ping(self):
        ...
        try:
            ...
            delay = self._response_handler(packet_id, started_at)
    ...
    def _checksum(self, src):
        ...
        while count < count_to:
            this_val = src[count + 1] * 256 + src[count]
            checksum += this_val
            checksum &= 0xffffffff
            count += 2
        if count_to < len(src):
            checksum += src[len(src) - 1]
            checksum &= 0xffffffff
    ...
    def _create_packet(self, packet_id):
        ...
        # Builds Real Header
        header = struct.pack(
            "bbHHh", 8, 0, socket.htons(self._checksum(
                header + bytes(data, encoding='utf8'))),
            packet_id, 1)
...
print("Destination: {}, return code: {}. RTT: {:.2f}ms".format(
    res['dest'], res['ret_code'], res['rtt']))
